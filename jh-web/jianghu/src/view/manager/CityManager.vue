...
export default {
  ...
  methods: {
    ...
    editorCity({row, col, record}) {
      if (row || row === 0) {
        if (col || col === 0) {
          record = this.worldObj.matrixMap[row][col]
          this.addForm.x = row;
          this.addForm.y = col;
        }
      }
        ...
        <table cellspacing="30">
          ...
          <tr v-for="row in worldObj.matrixMap.length" :key="row">
            <td v-for="col in worldObj.matrixMap[0].length"
                ...
                :key="col" @click="editorCity({row:row - 1,col:col - 1})">
              <a-tooltip :title="getCityObj(row - 1,col - 1).name " placement="left">
                <div class="td-info">
                  {{ getCityObj(row - 1, col - 1).name }}
                </div>
              </a-tooltip>
            </td>
          </tr>
